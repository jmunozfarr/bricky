...
def test_valid_static_fixtures_expose_bounded_playback(
    ...
    client, model_id, _source = imported_fixture(catalog_session_factory, tmp_path, name)
    ...
    occurrence = client.get(f"/api/models/{model_id}/instruction-occurrences/{root_id}?step=1")
...
def test_occurrence_payload_has_breadcrumbs_immediate_children_and_repeats(
    ...
    root = client.get(f"/api/models/{model_id}/instruction-occurrences/occ-000001?step=1").json()
    child = client.get(f"/api/models/{model_id}/instruction-occurrences/occ-000003?step=1").json()

    assert root["breadcrumbs"] == [{"occurrenceId": "occ-000001", "sourceSubmodelName": "main.ldr"}]
...
def test_derived_source_is_safe_ephemeral_and_preserves_original(
    ...
    response = client.get(f"/api/models/{model_id}/instruction-occurrences/occ-000001/source")
...
def test_unknown_occurrence_and_traversal_attempt_return_404(
    ...
    assert (
        client.get(f"/api/models/{model_id}/instruction-occurrences/occ-999999").status_code == 404
    )
    assert (
        client.get(f"/api/models/{model_id}/instruction-occurrences/occ-999999/source").status_code
        == 404
    )
    assert (
        client.get(
...
def test_cycle_and_safety_limit_return_flattened_fallback_state(
    ...
    cycle = cycle_client.get(f"/api/models/{cycle_id}/instruction-playback").json()

    assert cycle["available"] is False
    assert cycle["rootOccurrenceId"] is None
    assert cycle["issues"][0]["code"] == "recursive_submodel_cycle"
    assert (
        cycle_client.get(f"/api/models/{cycle_id}/instruction-occurrences/occ-000001").status_code
        == 409
    )
    ...
    limited_id = upload(limited_client, limited_source, "limited.mpd").json()["modelId"]
    limited = limited_client.get(f"/api/models/{limited_id}/instruction-playback").json()
...
def test_large_occurrence_navigation_payload_is_paginated(
    ...
    response = upload(client, generated_large_repeated_model(1_000), "generated-1000.mpd")
    assert response.status_code == 201
    model_id = response.json()["modelId"]

    summary = client.get(f"/api/models/{model_id}/instruction-playback")
    occurrence = client.get(f"/api/models/{model_id}/instruction-occurrences/occ-000001?step=1")
