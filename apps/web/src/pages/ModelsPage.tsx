...
export default function ModelsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(`/models/${created.modelId}?return=${encodeURIComponent(returnSearch)}`);
    } catch (error: unknown) {
  ...
  return (
    <section className="page-panel" aria-labelledby="models-title">
      <div className="page-heading catalog-heading">
        <div><p className="eyebrow">Local files</p><h2 id="models-title">Imported models</h2></div>
        <p>Original source bytes are preserved</p>
      </div>

      <form className="model-upload" onSubmit={(event) => void submit(event)}>
        <label><span>Model file</span><input type="file" accept=".ldr,.mpd" disabled={uploading} onChange={(event) => setFile(event.currentTarget.files?.[0] ?? null)} /></label>
        <label><span>Model name (optional)</span><input value={name} maxLength={256} disabled={uploading} onChange={(event) => setName(event.currentTarget.value)} placeholder="Uses the filename by default" /></label>
        <button type="submit" disabled={uploading}>{uploading ? "Importing…" : "Import model"}</button>
        {file && <p className="file-preview">{file.name} · {formatFileSize(file.size)}</p>}
        {uploadError && <p className="inline-error" role="alert">{uploadError}</p>}
      </form>

      <div className="catalog-filters">
        <label><span>Search models</span><input type="search" value={searchInput} onChange={(event) => setSearchInput(event.currentTarget.value)} placeholder="Name or original filename" /></label>
        <label><span>Import status</span><select value={status} onChange={(event) => updateParams("status", event.currentTarget.value, true)}><option value="">All statuses</option><option value="ready">Ready</option><option value="ready_with_warnings">Ready with warnings</option><option value="failed">Failed</option></select></label>
      </div>

      {results.kind === "loading" && <div className="page-message">Loading models…</div>}
      {results.kind === "error" && <div className="error" role="alert">{results.message}</div>}
      {results.kind === "ready" && <>
        <p className="results-count">{results.data.totalItems.toLocaleString()} models</p>
        {results.data.items.length === 0 ? <div className="empty-state">{query || status ? "No models match these filters." : "No models have been imported."}</div> :
          <div className="models-grid">{results.data.items.map((model) => <article className="model-card" key={model.modelId}>
